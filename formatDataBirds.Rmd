...
counties_taxonomic = 
  ...
  select(county, All, 2:40, perc_not_secure_state, perc_not_secure_global) %>% 
...
taxonomic_security <- biodiversity_security %>%
  ...
  select(county, taxonomic_subgroup, perc_not_secure_state, perc_not_secure_global) %>% 
...
```{r}
# I followed this tutorial: https://www.r-bloggers.com/2017/10/county-level-choropleth-in-plotly-and-r/
...
ggplot(choropleth, aes(long, lat, group = subregion)) +
  geom_polygon(aes(fill = n_species), colour = alpha("black", 1/2), size = 0.1)  +
  scale_fill_viridis_c() +
  theme_void()
```

```{r}
ggplot(counties_derived, aes(x = human_pop_county, y = perc_not_secure_state)) + 
    scale_x_log10() +
...
ggplot(counties_derived, aes(x = human_pop_county, y = n_species)) + 
    scale_x_log10() +
    geom_point(color = "#00BFC4", alpha = 0.6, size = 1) +
    theme_minimal() +
    theme(legend.position = "none")

ggplot(counties_derived, aes(x = density, y = n_species)) + 
    scale_x_log10() +
    geom_point(color = "#00BFC4", alpha = 0.6, size = 1) +
    theme_minimal() +
    theme(legend.position = "none")

ggplot(counties_derived, aes(x = total_area, y = n_species)) + 
    geom_point(color = "#00BFC4", alpha = 0.6, size = 1) +
    theme_minimal() +
    theme(legend.position = "none")

ggplot(counties_derived, aes(x = n_species, y = perc_not_secure_state)) + 
    geom_point(color = "#00BFC4", alpha = 0.6, size = 1) +
    theme_minimal() +
    theme(legend.position = "none")
```