...
species_by_subgroup <- species_by_subgroup %>%
...
write_csv(species_by_subgroup, "./data/species_by_subgroup.csv")
```
...
```{r}
counties_nameless = counties_derived %>% 
  select(-county)

# Note that density = human_pop_county / total_area
model = lm(perc_not_secure_state ~ ., data = counties_nameless)
summary(model)
```